...
import hdtx.androidsdk.data.ErrorResult;
...
//        setContentView(R.layout.hd_web);
//        mWebView = findViewById(R.id.hd_id_WebView);
//        ImageView mBack = (ImageView)findViewById(R.id.hd_pay_back);
        ...
        mWebView = (WebView) findViewById(getApplication().getResources().getIdentifier("hd_id_WebView", "id",
                getApplication().getPackageName()));
        ImageView mBack = (ImageView) findViewById(getApplication().getResources().getIdentifier("hd_pay_back", "id",
                getApplication().getPackageName()));
        ...
        room_url = intent.getStringExtra("url");
...
//	    cookieManager.setCookie(room_url, Constant.EASOUTGC + "=" + token);//cookies是在HttpClient中获得的cookie
        ...
        mWebViewClient = new WebViewClient() {
				...
				/*if (url.startsWith(back_url)) {
					...
					if (back_num == 0) {
						if ("success".equals(status)) {
							Starter.getInstance().payCallback(0);
							mActivity.finish();
						} else if ("fail".equals(status)) {
							ESToast.getInstance().ToastShow(mActivity, "支付失败");
							Starter.getInstance().payCallback(1);
							mActivity.finish();
						} else {
							Starter.getInstance().payCallback(2);
							mActivity.finish();
						}
						back_num = 1;
					}
            ...
            public void onPageStarted(WebView view, String url, Bitmap favicon) {
                ...
                if (url.startsWith("app://xpayment")) {
                    mActivity.finish();
                }
			/*	if (url.startsWith(back_url)) {
					String status = Tools.getParam(url, "status");
            ...
            public void onReceivedSslError(WebView view, final SslErrorHandler handler, SslError error) {
                AlertDialog.Builder builder = new AlertDialog.Builder(ESPayWebActivity.this);
...
//			    cookieManager.setCookie(url, Constant.EASOUTGC + "=" + token);//cookies是在HttpClient中获得的cookie
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == getApplication().getResources().getIdentifier("hd_id_BackBtn", "id",
                getApplication().getPackageName())) {
            mActivity.finish();
        }
    }
